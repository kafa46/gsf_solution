...
from anomaly_ai.modeling.networks.backbone import (
...
class DRA(nn.Module):
    ...
    def forward(self, image, label: int = None):
        image_pyramid = [[] for _ in range(self.cfg.total_heads)]
        for s in range(self.cfg.n_scales):
            ...
            if self.training:
                ...
                comparison_scores = self.composite_head(feature, ref_feature)
            all_scores = [normal_scores, abnormal_scores, dummy_scores, comparison_scores]
